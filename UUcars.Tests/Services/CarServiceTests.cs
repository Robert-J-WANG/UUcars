...
public class CarServiceTests
{
    private static CarService CreateService(
        ...
        FakeStorageService? storageService = null) // ← 新增可选参数
    {
        return new CarService(
            ...
            storageService ?? new FakeStorageService() // ← 新增传入
        );
    }
    ...
    public async Task UpdateAsync_WhenDraftAndOwner_ShouldUpdateFields()
    {
        ...
        // Act
        var result = await service.UpdateAsync(1, 10, request);
    ...
    public async Task UpdateAsync_WhenPendingReview_ShouldThrowCarStatusException()
    {
        ...
        // Act + Assert
        await Assert.ThrowsAsync<CarStatusException>(() => service.UpdateAsync(1, 10, request));
    }
